...
public class InvalidDestinationCityState implements State {
    @Override
    public State next(String userInput, Request request) {
        return new SelectSpecificDestinationState().next(userInput, request);
    }

    @Override
    public SendMessage generateMessage(Request request) {
        SendMessage message = new SendMessage();
        message.setText("Не удалось распознать город. Введите название города заново");
        return message;
    }
}
